...
class EmailVerificationController extends Controller
{
    public function verify(Request $request){
        ...
        if (!$email || !$token) {
            ...
            throw new InvalidRequestException('验证链接不正确'); // 3.4. 优雅地处理异常 修改
        }
        // 从缓存中读取数据，我们把从 url 中获取的 `token` 与缓存中的值做对比
        // 如果缓存不存在或者返回的值与 url 中的 `token` 不一致就抛出异常。
        if ($token != Cache::get('email_verification_'.$email)) {
            //throw new \Exception('验证链接不正确或已过期');
            throw new InvalidRequestException('验证链接不正确或已过期'); // 3.4. 优雅地处理异常 修改
        }
        ...
        if (!$user = User::query()->where('email', $email)->first()) {
            ...
            throw new InvalidRequestException('用户不存在'); // 3.4. 优雅地处理异常 修改
        }
    ...
    public function send(Request $request){
        ...
        if ($user->email_verified) {
            ...
            throw new InvalidRequestException('你已经验证过邮箱了'); // 3.4. 优雅地处理异常 修改
        }
